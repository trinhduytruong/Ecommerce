...
const VoucherTable = ({ categories, openCategoryModal, setCategoryToDelete, setShowDeleteModal }) => {
    return (
        <Table striped bordered hover responsive="md" className="text-center align-middle">
            <thead className="table-light">
            <tr>
                <th>#</th>
                <th className="text-start">Voucher</th>
                <th className="text-start">Code</th>
                <th className="text-start">Mô tả</th>
                <th className="text-start">Giá trị</th>
                <th>Trạng thái</th>
                <th>Ngày tạo</th>
                <th>Thao tác</th>
            </tr>
            ...
            { categories?.length > 0 ? categories?.map((category, index) => (
                <tr key={category?.id}>
                    ...
                    <td className="text-start">{category?.code}</td>
                    <td className="text-start">{category?.description}</td>
                    <td className="text-start">{formatPrice(category?.price)}</td>
